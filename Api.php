<?php  
 ...
 class Api extends Rest {  
   ...
   const usuario_db = "n9000250";  
   const pwd_db = "Sistemashmjavi89";  
   const nombre_db = "n9000250_chat";  
   private $_conn = NULL;  
   private $_metodo;  
   private $_argumentos;  
   ...
   private function conectarDB() {  
     ...
     try {  
       ...
       require_once 'Conexiones/conexionPostgresql.php';
   ...
   private function usuarios() {  
     ...
     $query = $this->_conn->query("SELECT * FROM usuario");  
    ...
    //el constructor del padre ya se encarga de sanear los datos de entrada  
       ...
       if (!empty($email) and !empty($pwd)) {  
         ...
         if (filter_var($email, FILTER_VALIDATE_EMAIL)) {  
           ...
           $query = $this->_conn->prepare("SELECT id, nombre, email, fecha_registro FROM usuarios WHERE   
   ...
   private function actualizarNombre($idUsuario) {  
     ...
     if (isset($this->datosPeticion['nombre'])) {  
       ...
       if (!empty($nombre) and $id > 0) {  
         $query = $this->_conn->prepare("update usuarios set nombre=:nombre WHERE id =:id");  
   ...
   private function crearUsuario() {  
     ...
     if (isset($this->datosPeticion['nombre'], $this->datosPeticion['email'], $this->datosPeticion['pwd'])) {  
       ...
       if (!$this->existeUsuario($email)) {  
         $query = $this->_conn->prepare("INSERT into usuarios (nombre,email,password,fecha_registro) VALUES (:nombre, :email, :pwd, NOW())");  
         $query->bindValue(":nombre", $nombre);  
         $query->bindValue(":email", $email);  
         $query->bindValue(":pwd", sha1($pwd));  
         $query->execute();  
